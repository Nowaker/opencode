...
export function triggers(target: SessionMaintenanceSql.Target, keys: readonly Key[]): readonly string[] {
  ...
  const collision = sql.join(keys.map(key => sql`(${sql.join(key.map(column =>
  ...
  return (["INSERT", "UPDATE"] as const).flatMap(action => [
    dialect.sqlToQuery(sql`
      ...
      WHEN EXISTS(SELECT 1 FROM ${sql.identifier(target.table)} existing, session_maintenance_fence blocked
        WHERE (${collision}) AND blocked.session_id IN (${refs}))
    ...
    dialect.sqlToQuery(sql`
      ...
      WHEN EXISTS(SELECT 1 FROM main.${sql.identifier(target.table)} existing, main.session_maintenance_generation g
        WHERE (${collision}) AND g.session_id IN (${refs})
